...
public class RedisUtil {
    ...
    public static String getStr(String key) throws Exception {
        try {
            ...
            logger.error(RedisUtil.class.getName() + ".getStr" + e, e);
            throw e;
        }
    }

    public static String getStr(int db, String key) throws Exception {
        try {
    ...
    public static void setStr(int db, String key, String value) {
    ...
    public static void setExpire(int db, String key, long timeout) {
    ...
    public static Long delete(int db, String key) {
    ...
    public static Long delete(int db, String key, String hashKey) {
        return stringRedisTemplate.execute(new RedisCallback<Long>() {

            @Override
            public Long doInRedis(RedisConnection redis) throws DataAccessException {
                redis.select(db);
                return redis.hDel(key.getBytes(), hashKey.getBytes());
            }
        });
    }
    ...
    public static void setList(String key, List<String> list) {
        ...
        stringRedisTemplate.opsForList().rightPushAll(key, arr);
    }
    ...
    public static Map<String, String> getAllHash(int db, String hashKey) {
        Map<byte[], byte[]> map = stringRedisTemplate.execute(new RedisCallback<Map<byte[], byte[]>>() {
    ...
    public static void setLogger(Logger logger) {
        RedisUtil.logger = logger;
    }

    public static Set<String> keys(int db, String pattern) {
        Set<byte[]> set = stringRedisTemplate.execute(new RedisCallback<Set<byte[]>>() {
    ...
    public static String hget (int db,String key,String hashKey){
        try {
            return stringRedisTemplate.execute(new RedisCallback<String>() {
                ...
                public String doInRedis(RedisConnection redis) throws DataAccessException {
                    ...
                    String value = "";
                    try {
                            byte[] hGet = redis.hGet(key.getBytes(), hashKey.getBytes());
                            value = new String(hGet,"UTF-8");
                    } catch (UnsupportedEncodingException e) {
                        logger.error(e.getMessage(), e);
                    } catch (Exception e) {
                        logger.error(e.getMessage(), e);
            ...
            e.printStackTrace();
            logger.error(RedisUtil.class.getName() + ".hget" + e, e);
            throw e;
        }
    }
    
    public static String hset (int db,String key,String hashKey,String value){
        try {
            return stringRedisTemplate.execute(new RedisCallback<String>() {
                Boolean result ;
                @Override
                public String doInRedis(RedisConnection redis) throws DataAccessException {
                    redis.select(db);
                    try {
                        delete(db, key, hashKey);
                        this.result = redis.hSet(key.getBytes(), hashKey.getBytes(), value.getBytes());
                    } catch (Exception e) {
                        logger.error(e.getMessage(), e);
                    }
            ...
            e.printStackTrace();
            logger.error(RedisUtil.class.getName() + ".hset" + e, e);
            throw e;
        }
    }
        ...
        try {
            return stringRedisTemplate.execute(new RedisCallback<Long>() {
                ...
                public Long doInRedis(RedisConnection redis) throws DataAccessException {
                    redis.select(db);
                    try {
                            this.result = redis.lPush(key.getBytes(),value.getBytes());
                    } catch (Exception e) {
                        logger.error(e.getMessage(), e);
                    }
                    return this.result;
